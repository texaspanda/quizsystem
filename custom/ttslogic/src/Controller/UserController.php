...
class UserController extends ControllerBase {
  ...
  public function buildTrainingsList(UserInterface $user) {
    ...
    $rows = array_map(function ($row) use ($user) {
      ...
      $gid = $row->gid;
      $name = $row->name;
      $query = \Drupal::database()->select('opigno_group_content', 'opc');
      $query->fields('opc', ['entity_id']);
      $query->condition('opc.group_id', $gid);
      $query->condition('opc.group_content_type_id', 'ContentTypeModule');
      $result = $query->execute()->fetchCol();
      $score = 0;
      if ($result) {
        $opigno_module = OpignoModule::load(reset($result));
        $score = ttslogic_get_user_module_score($opigno_module, $user);
      }
